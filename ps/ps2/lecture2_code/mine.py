...
# global variable needed for fast parsing
...
def count_frequency(word_list):
    ...
    # return L
    """
    new implementation, return dictionary with word as key and count as value
    """
    # ret = {}
...
def merge(left, right):
    ...
    while l_index < len(left) and r_index < len(right):
        if left[l_index] < right[r_index]:
            ret.append(left[l_index])
            l_index += 1
        else:
            ret.append(right[r_index])
            r_index += 1
    if l_index < len(left):
        ret.append(left[l_index])
...
def inner_product(D1,D2):
    ...
    #         j += 1
    #     elif L1[i][0] < L2[j][0]:
    #         # word in L1 but not L2
    #         i += 1
    #     else:
    #         # word not in L1 but in L2
    #         j += 1
    # return sum
